...
server.registerTool(
  'carsxe_specs',
  {
    ...
    inputSchema: {
      ...
      deepdata: z.string().optional().describe('Set to "true" for extended data'),
  ...
  async ({ vin, deepdata, disableIntVINDecoding }) => {
    try {
      const data = await apiGet('/specs', { vin, deepdata, disableIntVINDecoding });
      return textResult(data);
    } catch (err) {
      return errorResult(err);
    }
  ...
  'carsxe_plate',
  {
    ...
    inputSchema: {
      ...
      state: z
        .string()
        .optional()
        .describe('2-letter US state code for improved accuracy (e.g., CA, TX)'),
      district: z.string().optional().describe('Region within the country'),
    },
  },
  async ({ plate, country, state, district }) => {
    try {
      const data = await apiGet('/v2/platedecoder', { plate, country, state, district });
      return textResult(data);
    } catch (err) {
      return errorResult(err);
    }
...
server.registerTool(
  'carsxe_images',
  {
    ...
    inputSchema: {
      ...
      transparent: z.string().optional().describe('Set to "true" for transparent background'),
      angle: z.string().optional().describe('Image angle (e.g., front, rear, side)'),
  ...
  async ({ make, model, year, trim, color, transparent, angle, photoType, size, license }) => {
    try {
...
server.registerTool(
  'carsxe_intvin',
  {
    ...
    inputSchema: {
      vin: z.string().describe('The international Vehicle Identification Number'),
    },
  },
  async ({ vin }) => {
    try {
...
server.registerTool(
  'carsxe_ocr',
  {
    ...
    inputSchema: {
      imageUrl: z.string().url().describe('Publicly accessible URL of an image containing a VIN'),
    },
  },
  async ({ imageUrl }) => {
    try {
  ...
  'carsxe_plateocr',
  {
    ...
    inputSchema: {
      imageUrl: z
        .string()
        .url()
        .describe('Publicly accessible URL of an image containing a license plate'),
    },
  },
  async ({ imageUrl }) => {
    try {
...
server.registerTool(
  'carsxe_ymm',
  {
    ...
    inputSchema: {
      ...
      trim: z.string().optional().describe('Specific trim level (e.g., LE, XLE)'),
    },
  },
...
server.registerTool(
  'carsxe_obd',
  {
    ...
    inputSchema: {
      code: z
        .string()
        .describe('The OBD-II diagnostic trouble code (e.g., P0300, C1234, B0001, U0100)'),
    },
  },
  async ({ code }) => {
    try {
